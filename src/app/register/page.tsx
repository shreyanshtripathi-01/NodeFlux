        ...
        <div className="flex w-[460px] flex-col gap-8 rounded-xl border border-border bg-panel px-10 py-10 shadow-2xl shadow-background">
          <div className="flex flex-col gap-6">
            ...
            <div className="space-y-2">
              <h1 className="font-headings text-2xl font-semibold tracking-tight text-panel-foreground">{t('Create an account')}</h1>
              <p className="text-sm text-muted-foreground">{t('Start building workflows today')}</p>
            </div>
          </div>
          ...
          <form className="flex flex-col gap-5">
            ...
            <button formAction={signup} className="flex min-h-12 items-center justify-center rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground hover:opacity-90 transition-opacity mt-2 w-full">
              {t('Sign Up')}
            </button>
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <Link href="/login" className="text-foreground hover:underline">{t('Log in')}</Link>
          </div>
        </div>
      </div>
    </div>
